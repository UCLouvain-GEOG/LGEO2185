---
title: "LGEO2185: Session ? -- Model validation and tuning"
...
::: callout-note
This session assumes proficiency with geographic data analysis, basic R, for example gained by studying the contents and working through the previous session. A familiarity with Generalized Linear Models (GLM) is recommended.
:::

## 1. Introduction + Why do we validate models?
...
When we fit a model, the key question is:

> How well will it predict *new, unseen data*?

A model can look excellent on the same data used for training (in-sample), yet perform poorly on new data (overfitting, leakage, or mismatch between training and prediction conditions).

**Validation** is therefore about *generalization*, not goodness-of-fit.

### Common validation strategies

-   **Apparent performance** (“train = test”): optimistic baseline.
-   **Hold-out split** (e.g., 70/30): simple but noisy (depends on the split).
...
The data used in this tutorial is derived from Lovelace et al.: Conceptual explanations in this document are inspired by Geocomputation with R (Lovelace et al.), used under a CC BY-NC-ND 4.0 license. (https://r.geocompx.org/)

> Tip: use 'install.packages' when needed.
...
We model a binary outcome `lslpts` with a binomial GLM. A Generalized Linear Model (GLM) extends ordinary linear regression to situations where the response variable is not continuous and normally distributed. In our case:
...
::: callout-note
Spatial distribution mapping is one very important outcome of a model (Figure 12.2). Even more important is how good the underlying model is at making them since a prediction map is useless if the model’s predictive performance is bad.
:::
...
Now, that isnt a bad fit! Note however that this is a very optimistic estimation of the model predictions since we have computed it on the complete dataset. In the sections below we address this issue.
...
![Figure: Train/Test Split illustration (source: https://www.datacamp.com/tutorial/k-fold-cross-validation)](https://media.datacamp.com/cms/google/ad_4nxfsjreoulmuvlpadvwouhqlmloap1c66vxmviqds-gho6xi2qaukhwv3o6s1erhbw5x48cg3uqkcpxf15rsc9qi0dpxpddxxh8udhhw8reuncf7zp7z004lye6fk7rylwmlhvxe_ctuxtuxkuwg3zgdafqc.jpeg)
...
The higher AUC on the training set reflects optimistic in-sample performance, while the test AUC provides a more realistic estimate of predictive skill on unseen data.
...
![Figure: Principle of cross-validation. The average performance for the 5 models than represents the overall performance after cross-validation (source: https://www.datacamp.com/tutorial/k-fold-cross-validation).](https://media.datacamp.com/legacy/v1718738336/image_0bb32b40f1.jpg)

In basic R, we can implement this functionality as follows:
...
::: callout
Spatial CV changes the validation question from:

> How well do I predict new *observations*?

to:

> How well do I predict at *new locations*?

This yields a more realistic estimate for spatial generalization.
:::
...
Now that is all good, but how do we know how many blocks we should choose? 
...
::: {.callout-tip collapse="true"}
Answer (click to expand) Because the model is evaluated on the same data used to fit it, leading to overly optimistic performance that does not reflect prediction at new locations.
:::

> **Question:** What question does spatial CV answer that random CV does not?

::: {.callout-tip collapse="true"}
Answer (click to expand) Spatial CV estimates performance at *new locations*, not just new observations close to the training data.
:::

> **Question:** Why is spatial autocorrelation assessed on model residuals?

::: {.callout-tip collapse="true"}
Answer (click to expand) Residuals represent unexplained structure. Autocorrelation in residuals indicates spatial dependence that can leak information between training and test data.
:::
...
Let's first split the dataset we were working with `lsl`into training and testdata. This split can be done automatically using the `initial_split`function from `rsample`. Note that we use `strata`argument to sample randomly within the stratification variable (`lslpts`).
...
The output of `lsl_split` is an object that contains the split information as well as the original dataframe.
...
Now we can put the model and recipe together into a worklfow. This can be done by using `workflow()`(from the `workflows`package) and by adding the recipe and model to it. Nice!
