...
pacman::p_load("raster", "sf", "tictoc", "tidyverse", "geodata", "terra","rasterVis","leaflet")
```
...
The `terra` package is a modern replacement of the `raster`/`sp` packages, with cleaner classes (`SpatRaster`, `SpatVector`). It is designed to be faster and more efficient, especially for large datasets. Key improvements brought by `terra` include:
* C++ core: Heavy raster/vector operations are implemented in C++ via R interfaces. This results in significant speedups compared to pure R implementations.
* Chunked I/O: Reads/writes in blocks so you can process rasters much larger than RAM; intermediate results stream to disk automatically.
* On-disk outputs: Most transforms can write directly to files, avoiding large in-memory objects and keeping memory bounded.
...
library(geodata);library(rasterVis)
