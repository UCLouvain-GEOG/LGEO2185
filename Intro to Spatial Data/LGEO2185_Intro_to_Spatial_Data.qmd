...
Unlike vector data, which preserves the boundaries of individual features, raster data generalizes an area into cells based on its dominant feature. In this process, unique geographic entities are reduced to collections of pixels with shared values, sacrificing their distinct identities.

![](images/raster.png)

Illustration showing the difference between vector and raster features (@esri_raster_2021)

Because managing massive grids of identical cells requires significant computational power, R utilizes the `terra` package has functions for creating, reading, manipulating, and writing raster data. The package provides, among other things, general raster data manipulation functions that can easily be used to develop more specific functions. For example, there are functions to read a chunk of raster values from a file or to convert cell numbers to coordinates and back. The package also implements raster algebra and many other functions for raster data manipulation.

The `terra` package is a modern replacement of the `raster`/`sp` packages, with cleaner classes (`SpatRaster`, `SpatVector`). It is designed to be faster and more efficient, especially for large datasets. Key improvements brought by `terra` include: \* C++ core: Heavy raster/vector operations are implemented in C++ via R interfaces. This results in significant speedups compared to pure R implementations. \* Chunked I/O: Reads/writes in blocks so you can process rasters much larger than RAM; intermediate results stream to disk automatically. \* On-disk outputs: Most transforms can write directly to files, avoiding large in-memory objects and keeping memory bounded.
...
[![sf cheatsheet](images/sf_cheat1.png){fig-align="center"}](https://rstudio.github.io/cheatsheets/sf.pdf)

[![sf cheatsheet (page 2)](images/sf_cheat2.png){fig-align="center"}](https://rstudio.github.io/cheatsheets/sf.pdf)

::: callout-warning
When working with `sf` objects, summarizing functions can unintentionally create unions of geometries. This is because the `geometry` column, which stores spatial data, is treated as part of the dataset and will be summarized too. While this behavior can be desirable for some use cases, it may not always be what you want...
...
crdref <- "+proj=longlat +datum=WGS84"
df <- as.data.frame(c("Goma", "Bukavu", "Bujumbura"))
ptv <- vect(lonlat, atts = df, crs = crdref)
```
