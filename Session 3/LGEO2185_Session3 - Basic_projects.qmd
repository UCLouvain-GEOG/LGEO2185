---
title: "LGEO2185: Session 3 -- R Project Structure & Workflow"
...
setwd("C:/Users/Antoine Stevens/OneDrive - McKinsey & Company/LGEO/Session 3/")
here::i_am("LGEO2185_Session3 - Basic_projects.qmd")
```
...
You could then run `source("R/load_librarie.R")` at the beginning of all your scripts to load libraries automatically
...
# Example usage
raw_data <- read_csv(here(raw_data_path, "data.csv"))
...
# Save
saveRDS(raw_data_sampled, file = here(processed_data_path, "sampled_data.Rds"))
```
...
folders <- c(
    ...
    file.path(base_dir, "output", "plots")
)
...
Let us just use an example to test our code (ie the CNRM-CM6-1 model based on scenario 585 for the period 2041-2060)^[See the course LGEO1232 'Le Climat et ses Changements' for more info ;-)].
...
writeRaster(delta_annual,
    filename = here(
        "data", "processed",
        "projected_difference_annual_precipitation.tif"
    ),
    overwrite = T
)
```
...
```{r}
# Check file size
file_size <- file.info(here("data", "processed", "projected_difference_annual_precipitation.tif"))$size / (1024^2) # Convert to MB
file_size

# Ok, now use another compression method. (writeRaster uses "LZW" by default)
file_name <- here(
    "data", "processed",
    "projected_difference_annual_precipitation_DEFLATE.tif"
)
writeRaster(delta_annual,
    filename = file_name,
...
file_name <- here(
    "data", "processed",
    "projected_difference_annual_precipitation_SCALED.tif"
)
writeRaster(delta_annual,
    filename = file_name,
    overwrite = TRUE,
